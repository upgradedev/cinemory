"""B2-backed job store for async reel generation (the submit + poll pattern).

A reel generation can run for minutes (a real Kling render averages ~242s; see
``deploy/CLOUDRUN.md``), which is longer than the Firebase Hosting proxy cap
(~60s) and comparable to Cloud Run's own request ceiling. ``POST /reels/jobs``
(see ``cinemory.api``) avoids blocking on that: it validates the request,
writes a "queued" status object, starts the real generation in a background
thread, and returns immediately with a job id. ``GET /reels/jobs/{job_id}``
polls that status back.

A job's state is a small JSON object stored through the SAME
:class:`~cinemory.ports.StorageBackend` the rest of the app already uses (B2 in
live mode, the in-memory fake offline), under ``jobs/<job_id>/status.json`` —
NOT a separate in-process registry. That is what makes status pollable across
Cloud Run instances: the object store, not any one process's memory, is the
source of truth, so a scale-to-zero instance that never saw the submit can
...
accepted the submit** (see ``cinemory.api._run_job``). There is no external
queue and no separate worker fleet. A client that keeps polling keeps that
instance warm (a request in flight resets Cloud Run's idle-scale-down clock),
so in practice the job completes. But if that specific instance is scaled down
mid-job — e.g. nobody polls for a while, or Cloud Run reclaims it for another
reason — the in-flight generation is lost with no automatic retry; the stored
status simply stops advancing past whatever it last reached. This is an
acceptable, deliberate tradeoff for a demo, not a production job queue. A
...
_log = logging.getLogger("cinemory.jobs")

#: Every job status object lives under this prefix, in its own id-named folder
#: — a namespace that can never collide with a reel's own
#: ``<name>/<kind>/<sha2>/<sha256>/<file>`` keys (different segment count; see
#: ``cinemory.keys.make_key``).
_KEY_PREFIX = "jobs"
...
def _key(job_id: str) -> str:
    ...
    # (see cinemory.keys.safe_component), so a hostile id can never be shaped
    # into a key outside this job's own prefix. A legitimate id is untouched
    # by this (it is already within the safe charset).
...
def read(storage: StorageBackend, job_id: str) -> dict | None:
    ...
    Honest-degrade, mirroring ``cinemory.api.verify_reel``'s "unreadable bytes
    never 500" contract: a job id that was never created, a storage read error,
    corrupt JSON, or (defensively) JSON that isn't an object all read the same
    way — ``None`` — so the caller (``GET /reels/jobs/{job_id}``) can turn any
    of them into one honest 404 rather than leaking a stack trace.
    """
    try:
...
def mark_failed(storage: StorageBackend, job_id: str, exc: BaseException) -> dict:
    ...
    # cinemory.api._run_reel's ``degrade_reason`` (same reasoning, same shape).
