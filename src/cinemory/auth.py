...
nothing new to keep secret. This module is the security root of Cinemory's
...
This module is intentionally standalone — nothing else in ``cinemory`` imports
it yet, and it does not touch ``api.py``. A later change wires
``uid_from_authorization`` into the API so an optional ``Authorization: Bearer
<token>`` header resolves a tenant; this PR builds and adversarially tests the
...
# itself mirrors the lazy-import / injectable-transport seam
# ``cinemory.connectors._http`` uses for the same reason, and its real-network
# lines are excluded from the coverage gate the same way
# ``RequestsTransport`` is there.
...
def _refresh_certs_cache(now: float) -> Mapping[str, str]:  # pragma: no cover - real network path
    ...
    excluded from the coverage gate the same way
    ``cinemory.connectors._http.RequestsTransport`` is.
    """
