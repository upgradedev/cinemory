"""Unit tests for the async job store (``cinemory.jobs``).

Driven directly against a :class:`~cinemory.adapters.fake_storage.FakeStorage`
(the same ``StorageBackend`` port the real B2 adapter implements) — no HTTP,
no threading, so these pin the module's own contract in isolation. The full
...
def test_mark_running_without_prior_create_seeds_a_base_status():
    ...
    cinemory.api.create_reel_job before the background thread starts)."""
    storage = FakeStorage()
...
def test_hostile_job_id_key_stays_sanitised_and_namespaced():
    ...
    codebase is (cinemory.keys.safe_component), even though in the real flow
