...
- "Ports-and-adapters: live↔offline is a one-line swap. The full suite runs green
  offline with zero credentials — 147 passing plus 2 environment-conditional
  skips — including a contract test that runs the adapter against the **real
  published Genblaze SDK**, so API drift fails CI."
