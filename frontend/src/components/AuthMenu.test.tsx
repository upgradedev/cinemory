...
describe("<AuthMenu /> — auth enabled, signed in", () => {
  ...
  it("opens the panel on click, showing My reels and Sign out", async () => {
    render(<AuthMenu onOpenLibrary={vi.fn()} />);
    const trigger = screen.getByRole("button", { name: "Ada Lovelace" });
    expect(trigger).toHaveAttribute("aria-expanded", "false");
    await userEvent.click(trigger);
    expect(trigger).toHaveAttribute("aria-expanded", "true");
    expect(screen.getByRole("button", { name: /my reels/i })).toBeInTheDocument();
    expect(screen.getByRole("button", { name: /sign out/i })).toBeInTheDocument();
  });

  it("calls onOpenLibrary and closes the panel when My reels is clicked", async () => {
    ...
    expect(screen.queryByRole("button", { name: /my reels/i })).not.toBeInTheDocument();
  });
