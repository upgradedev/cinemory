...
// module is imported unconditionally from a few other lib files (api.ts,
// provenance-verify.ts), but it never pulls Firebase's JS into the bundle by
// doing so: `firebase/app` and `firebase/auth` are ONLY ever reached through
// a dynamic import inside getFirebaseAuth(), and every exported function
// checks isAuthEnabled() (or, for getIdToken, relies on getFirebaseAuth()
// never being called) before that import can happen. A guest build (the
// default: no VITE_FIREBASE_* vars, exactly today's production config) never
// executes that branch, so it never fetches the Firebase chunk at all.
//
...
// (see the withAuth() helpers in lib/api.ts and lib/provenance-verify.ts,
// which call this on every request instead of storing a token).
 ...
 *  sign-in UI must never render — see AuthMenu/MyReels.
 *
