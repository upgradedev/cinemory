...
import { cinemoryApi, type LibraryReel } from "./api";
...
const REELS: LibraryReel[] = [
  { reel_name: "a", manifest_hash: "h", created_at: "2026-01-01T00:00:00Z", updated_at: null },
];
...
describe("useMyLibrary", () => {
  it("stays disabled (never fetches) when enabled=false", () => {
    const spy = vi.spyOn(cinemoryApi, "myLibrary");
  ...
  it("fetches the library once enabled=true", async () => {
    ...
    expect(result.current.data).toEqual(REELS);
  });

  it("does not retry on failure — a 401 would never succeed on retry", async () => {
    const spy = vi.spyOn(cinemoryApi, "myLibrary").mockRejectedValue(new Error("boom"));
    const { result } = renderHook(() => useMyLibrary(true), { wrapper: wrapper() });
...
describe("useDeleteMyData", () => {
  it("resolves the deleted count", async () => {
    vi.spyOn(cinemoryApi, "deleteMyData").mockResolvedValue({ deleted: 2 });
  ...
  it("invalidates the library query on success, so a mounted MyReels view refetches", async () => {
    vi.spyOn(cinemoryApi, "myLibrary").mockResolvedValue(REELS);
